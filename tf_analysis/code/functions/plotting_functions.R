...
plot_connections_per_gene <- function(s){
    boxplot(
        list(
            "stemming\nconnections" = s$connection_per_TF,
            "received\nconnections" = s$connection_per_TG
        ),
        col = c("red","blue"),
...
plot_genes_ratioconnections <- function(s){
    boxplot(
        s$In_Out_per_Gene$ratio,
        main = "Genes by out/in\nratio of connections",
        col = "#ccabf4"
    )
}

plot_ratio_connections <- function(s){
    plot(
        seq(1:length(s$In_Out_per_Gene$ratio)),
        s$In_Out_per_Gene$ratio,
        main = "Gene behavior\n(types & amt of connections)",
        col = colorRampPalette(
            c("blue", "red")
            )(length(s$In_Out_per_Gene$ratio)),
        xlab = "",
...
plot_centrality_TFclass <- function(s,f){
    boxplot(
        s$Centrality_per_TFclass[order(names(s$Centrality_per_TFclass))],
        col = unique(f[order(names(f))]),
        main = "Centrality per TF class",
        ylab = "Centrality"
        )
}
...
plot_centrality_TDHK <- function(s){
    boxplot(
        s$Centrality_per_TransDev_HK[c(1,2)],
        col = c("purple","orange"),
        main = "Centrality of\ntrans-dev genes"
        )
}
